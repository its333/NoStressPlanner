...
export async function GET(req: NextRequest) {
  try {
    ...
    const cookieInfo = {
      ...
      sessionCookies: {
        ...
        attendeeSession: {
          ...
          preview: attendeeSession ? `${attendeeSession.substring(0, 20)}...` : null
        },
        anonymousSession: {
          hasValue: !!anonymousSession,
          preview: anonymousSession ? `${anonymousSession.substring(0, 20)}...` : null
        }
      },
      selectedSessionKey: attendeeSession || anonymousSession ? 
        `${(attendeeSession || anonymousSession)!.substring(0, 20)}...` : null
    };
    ...
    return NextResponse.json({ error: 'Failed to get cookie info' }, { status: 500 });
  }
}
