...
import { handleNextApiError, ValidationError, NotFoundError, ConflictError } from '@/lib/error-handling';
...
export const POST = rateLimiters.voting(async (req: NextRequest, context: { params: Promise<{ token: string }> }) => {
  try {
    ...
    console.log('🔍 Blocks API received data:', json);
    const parsed = blocksSchema.safeParse(json);
    
    if (!parsed.success) {
      console.log('❌ Validation failed:', parsed.error.flatten());
      throw new ValidationError('Invalid blocks data', parsed.error.flatten());
    }
    ...
    if (event.phase !== 'PICK_DAYS') {
      throw new ConflictError('Blocks can only be updated during PICK_DAYS phase');
    }
    ...
    try {
      ...
      console.warn('⚠️ JWT session error in blocks API, continuing without auth:', error);
    }
    ...
    if (!currentSession) {
      return NextResponse.json({ error: 'Join the event before blocking days' }, { status: 401 });
    }
    ...
    const normalizedDates = [...new Set(dates.map((date) => toUtcDate(date).toISOString()))];
    for (const iso of normalizedDates) {
      if (!isWithinRange(iso, event.startDate, event.endDate)) {
        return NextResponse.json({ error: 'Date outside event range' }, { status: 400 });
      }
    }
    ...
    const existingSet = new Set(existingBlocks.map((block) => toUtcDate(block.date).toISOString()));
    ...
    console.log('📡 Emitted blocks.updated event for event:', event.id);
