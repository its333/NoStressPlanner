...
export const POST = rateLimiters.general(
  async (req: NextRequest, context: { params: Promise<{ token: string }> }) => {
    ...
    debugLog('Final API: request received', json);
    const parsed = finalDateSchema.safeParse(json);
    if (!parsed.success) {
      debugLog('Final API: validation failed', parsed.error.flatten());
    ...
    debugLog('Final API: host detection summary', {
    ...
    if (!isHost) {
      return NextResponse.json(
        { error: 'Only the host can pick the final date' },
        { status: 403 }
      );
    }
    ...
    if (date && !isWithinRange(date, event.startDate, event.endDate)) {
      return NextResponse.json(
        { error: 'Final date must be within the event range' },
        { status: 400 }
      );
    }
    ...
    await emit(event.id, 'final.date.set', {
      date: updated.finalDate ? updated.finalDate.toISOString() : null,
    });
