...
export default function EventPageClient({ token }: { token: string }) {
  ...
  const { data, mutate, isLoading } = useSWR<ApiResponse>(`/api/events/${token}?refresh=${refreshKey}`);
  ...
  const forceRefresh = useCallback(() => {
    console.log('🔍 EventClient: Force refresh triggered');
  ...
  const [finalDateStatus, setFinalDateStatus] = useState<"idle" | "confirming" | "setting" | "success">("idle");
  ...
  useEffect(() => {
    ...
    if (data?.event?.showResultsToEveryone !== undefined) setShowResultsToEveryone(data.event.showResultsToEveryone);
    ...
    if (data?.event?.id) {
      const clearStaleCookies = () => {
        ...
        cookies.forEach(cookie => {
          ...
          if ((name === 'attendee-session' || name === 'anonymous-session') && value) {
        ...
        if (hasStaleCookie) {
          console.log('🔍 Cleared stale session cookies, refreshing page...');
          // Refresh the page to get fresh data without stale cookies
  ...
  useEffect(() => {
    ...
    console.log('Setting up real-time updates for event:', data.event.id);
    
    // Check Pusher availability
    const hasPusherConfig = !!(PUSHER_KEY && PUSHER_CLUSTER);
    console.log('🔍 Pusher Configuration Check:', {
    ...
    if (hasPusherConfig) {
      ...
      const eventHandlers = {
        "vote.updated": (_data: any) => {
          console.log('🗳️ Vote updated event received');
          mutate();
        },
        "blocks.updated": (_data: any) => {
          console.log('📅 Blocks updated event received');
          mutate();
        },
        "phase.changed": (data: any) => {
          console.log('🔄 Phase changed event received:', data?.phase);
          mutate();
        },
        "final.date.set": (_data: any) => {
          console.log('📅 Final date set event received');
          mutate();
        },
        "attendee.nameChanged": (_data: any) => {
          console.log('👤 Attendee name changed event received');
          mutate();
        },
        "attendee.joined": (_data: any) => {
          console.log('➕ Attendee joined event received');
          mutate();
        },
        "attendee.left": (_data: any) => {
          console.log('➖ Attendee left event received');
          mutate();
        },
        "showResults.changed": (_data: any) => {
          console.log('👁️ Show results changed event received');
          mutate();
        }
      };
      ...
      events.forEach((event) => {
        ...
        console.log('🔗 Bound to Pusher event:', event);
      });
      ...
      const poll = async () => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (currentTime - lastUpdateTime > 3000) { // 3 seconds
  ...
  const switchName = useCallback(async (newNameId: string) => {
    ...
    try {
      ...
      console.log('Before mutate - current state:', {
        unavailableDates,
        currentVote,
      ...
      console.log('After mutate - data refresh triggered');
      
      // Show success message if provided
      const responseData = await response.json();
      if (responseData.message) {
        console.log('Name switch success:', responseData.message);
      }
      
    } catch (error) {
      console.error('Error switching name:', error);
      setNameSwitchError(error instanceof Error ? error.message : 'Failed to switch name');
    }
  }, [data?.event?.id, token, mutate]);
  ...
  const handleDeleteEvent = useCallback(async () => {
    ...
    try {
      ...
      setHostActionError(err instanceof Error ? err.message : 'Failed to delete event');
    } finally {
  ...
  const submitVote = useCallback(
    async (inValue: boolean) => {
      console.log('Submitting vote:', { inValue, currentVote, token });
      ...
      try {
        ...
        console.log('Vote response:', { status: res.status, ok: res.ok });
        
        if (!res.ok) {
          const detail = await res.json().catch(() => ({}));
          setCurrentVote(previousVote);
          setVotingStatus("error");
          console.error("Vote failed:", detail?.error ?? "Failed to submit vote");
        ...
        console.log('Vote successful, mutating data...');
  ...
  const saveAvailability = useCallback(async () => {
    ...
    try {
      console.log('🔍 Sending availability data:', { dates: unavailableDates, anonymous });
      ...
      setAvailabilityError(error instanceof Error ? error.message : 'Failed to save availability');
      // Revalidate to get the correct data from server
      await mutate();
    } finally {
  ...
  const updatePhase = useCallback(
    async (phase: "PICK_DAYS" | "RESULTS" | "FINALIZED" | "FAILED" | "VOTE") => {
      ...
      try {
        ...
        setHostActionError('Failed to get security token. Please refresh the page.');
        return;
      }
  ...
  const updateFinalDate = useCallback(async (date: string | "clear") => {
    if (!data?.event?.id) return;
    ...
    await mutate();
      ...
      setHostActionError(error instanceof Error ? error.message : 'Failed to set final date');
    }
  }, [data?.event?.id, token, mutate]);

  const updateShowResultsToggle = useCallback(async (newValue: boolean) => {
    ...
    try {
      ...
      setHostActionError(error instanceof Error ? error.message : 'Failed to update toggle');
    } finally {
  ...
  return (
    <main className="container-page grid gap-6">
      ...
      {!isLoading && data && data.event.phase === "FINALIZED" && data.event.finalDate && (
        <section className="card bg-gradient-to-br from-green-50 via-blue-50 to-purple-50 border-2 border-green-200 shadow-lg">
          <div className="text-center py-8">
            ...
            <h2 className="text-3xl font-bold text-green-800 mb-4">Event Finalized!</h2>
            <p className="text-lg text-green-700 mb-8 max-w-2xl mx-auto">
              Congratulations! Your event has been successfully scheduled and all attendees have been notified.
            </p>
            ...
            <div className="bg-white rounded-2xl p-8 border-2 border-green-200 shadow-lg max-w-md mx-auto">
              ...
              <div className="text-lg text-slate-600 mb-4">Final Event Date</div>
      ...
      {data && (
        <section className="card grid gap-3">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex flex-col gap-2">
              <div className="flex flex-wrap items-center gap-3">
                <h1 className="text-2xl font-semibold text-slate-900">{data.event.title}</h1>
              </div>
              {hostActionError && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                  <div className="flex items-center gap-2">
                    <span className="text-red-600">⚠️</span>
                    <span className="text-sm text-red-800">{hostActionError}</span>
                    <button 
            ...
            <div className="flex flex-wrap items-center gap-2">
              {data.isHost && (
                <div className="flex flex-wrap items-center gap-2" aria-live="polite">
                  ...
                  {copyStatus === "copied" && <span className="text-xs text-green-600">Link copied</span>}
          ...
          {data.event.description && <p className="prose-muted">{data.event.description}</p>}
          {data.you && !needsJoin && (
            <div className="flex items-center gap-2 text-sm text-slate-600">
              <span>Joined as: <strong>{data.you.displayName}</strong> ({data.you.attendeeName.label})</span>
              <button 
                type="button" 
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="text-right text-xs text-slate-500">
              <p>Quorum {data.phaseSummary.inCount} / {data.phaseSummary.quorum}</p>
              <p>Deadline {data.phaseSummary.voteDeadline ? format(new Date(data.phaseSummary.voteDeadline), "PPpp") : 'No deadline'}</p>
            </div>
          </div>
          {data.event.finalDate && (
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <span className="text-sm font-semibold text-green-700">Event Finalized!</span>
                <span className="text-xs text-green-600">Final date: {format(new Date(data.event.finalDate), "EEEE, MMMM d, yyyy")}</span>
              </div>
            </div>
          )}
        </section>
      ...
      {needsLogin && <LoginPrompt message="This event requires a Discord login before you can interact." />}
      ...
      {needsJoin && data && (
        <PickNameCard
          ...
          defaultTz={browserTz}
          onJoined={() => {
            console.log('🔍 onJoined callback called, triggering force refresh');
      ...
      {!isLoading && data && !needsJoin && (data.event.phase === "PICK_DAYS" || data.event.phase === "RESULTS") && (
        <section className={`card ${currentVote !== true && data.event.phase === "PICK_DAYS" ? 'border-yellow-300 bg-yellow-50' : ''}`}>
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-slate-700">Your vote:</span>
              <span className={`text-sm px-2 py-1 rounded-full ${
                currentVote === true ? 'bg-green-100 text-green-800' : 
                currentVote === false ? 'bg-slate-100 text-slate-800' : 
                'bg-yellow-100 text-yellow-800'
              }`}>
                {currentVote === true ? 'I\'m in!' : currentVote === false ? 'Not this time' : 'Not voted'}
              </span>
              {votingStatus === 'voting' && (
              ...
              {currentVote !== true && data.event.phase === "PICK_DAYS" && (
                <span className="text-xs text-yellow-700 font-medium">⚠️ Vote "I'm in!" to block days</span>
              )}
            </div>
      ...
      {!isLoading && data && !needsJoin && data.event.phase === "VOTE" && (
        <section className="card grid gap-4">
          ...
          <h2 className="text-lg font-semibold text-slate-900">Are you in?</h2>
            {currentVote !== null && (
              <span className={`text-sm px-2 py-1 rounded-full ${
                currentVote === true ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-800'
              }`}>
          ...
          {votingStatus === 'success' && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-center">
              <span className="text-sm text-green-800">✓ Vote updated successfully!</span>
            </div>
          )}
        ...
        <section className="card grid gap-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-slate-900">Set Your Availability</h2>
                <p className="text-sm text-slate-600">Mark the days you cannot attend</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
          ...
          {currentVote !== true ? (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
              <div className="text-center">
                ...
                <h3 className="text-lg font-semibold text-yellow-800 mb-2">Vote Required to Set Availability</h3>
                <p className="text-yellow-700 mb-4">
                  You need to vote "I'm in!" above before you can mark unavailable days.
                </p>
                <p className="text-sm text-yellow-600">
                  Only attendees who are participating in the event can set their availability.
                </p>
              </div>
            </div>
          ) : (
          ...
          <BlockCalendar start={data.event.startDate} end={data.event.endDate} value={unavailableDates} onChange={setUnavailableDates} />
              </div>
              ...
              {availabilityError && (
                <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
                  <div className="flex items-center gap-2">
                    <span className="text-red-600">⚠️</span>
                    <span className="text-sm text-red-700 font-medium">{availabilityError}</span>
                  </div>
                </div>
              )}
                ...
                <div className="text-sm text-slate-600">
                  {unavailableDates.length > 0 ? (
                    <span className="text-orange-700 font-medium">
                      {unavailableDates.length} day{unavailableDates.length !== 1 ? 's' : ''} marked unavailable
                    </span>
                  ) : (
                    <span className="text-green-700 font-medium">All days available</span>
                  )}
                </div>
                <button 
      ...
      {!isLoading && data && (
        (data.event.phase === "RESULTS" || data.event.phase === "FINALIZED") ||
        (data.event.phase === "PICK_DAYS" && (data.isHost || showResultsToEveryone))
      ) && (
        <section className="card grid gap-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h2 className="text-lg font-semibold text-slate-900">Results & Suggestions</h2>
            {data.isHost && data.event.phase === "PICK_DAYS" && (
              <div className="flex items-center gap-2">
                <span className="text-sm text-slate-600">Show to everyone:</span>
                <button
                  type="button"
                  ...
                  onClick={() => updateShowResultsToggle(!showResultsToEveryone)}
                ...
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                      showResultsToEveryone ? 'translate-x-6' : 'translate-x-1'
                    }`}
                  />
                </button>
          ...
          <div className="grid gap-4">
            <div className="grid gap-2">
              <div className="rounded-lg bg-green-50 border border-green-200 p-4">
                <h3 className="font-semibold text-green-900 mb-2">🎯 Best Options</h3>
            ...
            <ResultsCalendar 
              ...
              onDateClick={data.event.phase === 'RESULTS' ? (date) => setFinalDraft(date) : undefined}
      ...
      {data?.isHost && (
        <section className="card grid gap-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900">Host Controls</h2>
              <p className="text-slate-600">Manage your event's progress and settings</p>
            </div>
          </div>
          ...
          {data.event.phase === 'PICK_DAYS' && (
            <div className="p-4 bg-white rounded-xl border border-purple-200">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h4 className="text-lg font-semibold text-slate-900">Availability Progress</h4>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                  <span className="text-sm font-medium text-slate-700">
                    {data.availabilityProgress.completedAvailability} of {data.availabilityProgress.totalEligible} participants have set their availability
                  </span>
                  <div className={`px-3 py-1 rounded-full text-xs font-semibold ${
                    ...
                    {data.availabilityProgress.isComplete ? '✅ Complete' : '⏳ In Progress'}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  {data.attendeeDetails
                    ...
                    .map((attendee) => (
                      <div key={attendee.id} className="flex items-center justify-between p-2 bg-white rounded-lg border border-slate-200">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-slate-900">{attendee.name}</span>
                          {attendee.isLoggedIn && (
                        ...
                        <span className={`text-xs font-medium ${
                          attendee.hasSetAvailability ? 'text-green-600' : 'text-yellow-600'
                        }`}>
                          {attendee.hasSetAvailability ? 'Availability Set' : 'Pending'}
                        </span>
                      </div>
                    ))}
                </div>
                  ...
                  <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-green-800">
                        All participants have set their availability! You can now move to the Results phase.
                      </span>
                    </div>
                  </div>
                ) : (
                  <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-blue-800">
                        You can move to Results at any time, even if not everyone has set their availability yet.
                      </span>
                    </div>
                  </div>
                )}
          ...
          <div className="grid gap-6">
            ...
            {data.event.phase === "PICK_DAYS" && (
                ...
                >
                  <span className="text-lg">{data.availabilityProgress.isComplete ? '🎉' : '📊'}</span>
                  {data.availabilityProgress.isComplete 
                    ? 'Ready for Results!' 
          ...
          {data.event.phase === "RESULTS" && (
            <div className="grid gap-6">
              <div className="text-center">
                ...
                <h3 className="text-2xl font-bold text-slate-900 mb-2">Pick Final Date</h3>
                <p className="text-slate-600 max-w-2xl mx-auto">
                  Click on any date in the calendar above to select it as your final event date. 
                  Consider availability and any external factors when making your choice.
                </p>
                {finalDateStatus === "success" && (
                  <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                    <span className="text-green-700 font-medium">✓ Final date set successfully!</span>
                  </div>
                )}
              </div>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-200">
                <div className="text-center mb-4">
                  <h4 className="text-lg font-semibold text-slate-900 mb-2">Selected Date</h4>
                  <p className="text-sm text-slate-600">Click on a calendar date above to make your selection</p>
                </div>
                ...
                <div className="text-center">
                  <div className="inline-flex items-center gap-3 px-6 py-4 bg-white rounded-xl border-2 border-slate-200 shadow-sm">
                    ...
                    {finalDraft && finalDraft !== "clear" ? (
                      ...
                      <span className="text-lg text-slate-400">No date selected</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button 
                  ...
                  disabled={!finalDraft || finalDraft === "clear" || finalDateStatus === "setting"}
                >
              ...
              {hostActionError && (
                <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-red-700 font-medium">Error setting final date</div>
                      <div className="text-sm text-red-600">{hostActionError}</div>
                    </div>
              </div>
            </div>
          )}
          ...
          <div className="pt-6 border-t border-slate-200">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold text-slate-900 mb-1">Danger Zone</h3>
                <p className="text-sm text-slate-600">Permanently delete this event and all its data</p>
              </div>
            <button 
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-8 max-w-lg w-full shadow-2xl">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-slate-900 mb-4">Confirm Final Date</h3>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 mb-8 border border-blue-200">
                ...
                <div className="text-slate-600 mb-3">Your selected event date</div>
              ...
              {hostActionError && (
                <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-red-700 font-medium">{hostActionError}</span>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-8 max-w-lg w-full shadow-2xl">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-slate-900 mb-4">Switch Name</h3>
              <p className="text-slate-600 mb-6 text-lg">
                Choose a name to represent in this event. If the name is already taken, you'll take over that person's progress.
              </p>
              ...
              <div className="space-y-3 mb-8">
                {data?.attendeeNames.map((name) => (
                  <button
                    ...
                    onClick={() => !name.claimedByLoggedUser && switchName(name.id)}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {name.takenBy ? (
                        name.claimedByLoggedUser ? (
                          ...
                          <span className="text-sm text-orange-600">Take Over</span>
                        )
                      ) : (
              ...
              {nameSwitchError && (
                <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg">
                  <div className="flex items-center gap-2">
                    <span className="text-red-600">❌</span>
                    <span className="text-sm text-red-700 font-medium">{nameSwitchError}</span>
                  </div>
                </div>
              )}
            </div>
