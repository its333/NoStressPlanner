      ...
      <section className="container-page grid gap-6">
        <header className="grid gap-1">
          <p className="text-sm text-slate-500">
            Welcome back, <span className="font-semibold text-slate-700">{session?.user?.name}</span>
          </p>
        ...
        {hosting.length > 0 && (
          <section className="grid gap-3">
            ...
            <EventGrid items={hosting} emptyLabel="You are not hosting any events yet." />
          </section>
        )}
        ...
        {attending.length > 0 && (
          <section className="grid gap-3">
            ...
            <EventGrid items={attending} emptyLabel="You have not joined any events yet." />
          </section>
        )}
...
function EventGrid({ items, emptyLabel }: { items: UserEventSummary[]; emptyLabel: string }) {
  if (items.length === 0) {
      ...
      {items.map((event) => {
        ...
        return (
          ...
          >
            ...
            <p className="mt-3 text-xs font-medium uppercase tracking-wide text-slate-500">{phaseLabel}</p>
          </Link>
        );
      })}
    </div>
