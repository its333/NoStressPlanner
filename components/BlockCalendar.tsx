...
export default function BlockCalendar({ start, end, value, onChange }: BlockCalendarProps) {
  ...
  const [dragState, dragHandlers] = useDragAndDrop(selected, onChange, isDateDisabled);

  const handlePointerDown = useCallback(
    (event: PointerEvent<HTMLButtonElement>, iso: string, disabled: boolean) => {
      if (disabled) return;
  ...
  return (
    <div className="grid gap-3">
      ...
      <div className="grid gap-1">
        ...
        <div className="grid grid-cols-7 gap-1">
          {daysInGrid.map((day) => {
            ...
            const withinRange = UtcValidator.isWithinRange(day, rangeStart, rangeEnd);
            const active = selected.has(iso);
            const muted = UtcValidator.shouldMute(day, viewMonthStart, new UtcDateRange(rangeStart, rangeEnd));
            const disabled = !withinRange;
            const inDragRange = isDateInDragRange(iso, dragState);

            let visualClasses = "rounded-lg border px-2 py-2 text-sm transition ";
            
            if (disabled) {
              // Out of range days - very dark grey
              visualClasses += "bg-slate-600 border-slate-700 text-slate-200 cursor-not-allowed";
            } else if (active) {
              // Blocked days (selected) - grey background
              visualClasses += "bg-slate-300 border-slate-400 text-slate-700 cursor-pointer";
            } else if (muted) {
              // Days from previous/next month
              visualClasses += "border-slate-200 bg-slate-50 text-slate-400 cursor-pointer hover:border-brand-200";
            } else if (inDragRange) {
              // Days in current drag range - enhanced visual feedback
              if (dragState.mode === 'add') {
                visualClasses += "bg-green-100 border-green-300 text-green-700 cursor-pointer";
              } else {
                visualClasses += "bg-red-100 border-red-300 text-red-700 cursor-pointer";
              }
            } else {
              // Available days - white background
              visualClasses += "border-slate-200 bg-white text-slate-700 cursor-pointer hover:border-brand-200";
            }
      ...
      <p className="text-xs text-slate-500">Click or drag across days to mark when you are unavailable.</p>
      ...
      <div className="grid gap-3 md:grid-cols-[8rem,1fr]">
          ...
          {years.map((year) => (
            <button
              ...
              className={`rounded-lg px-3 py-2 text-sm text-left transition ${
                year === activeYear ? "bg-brand-100 text-brand-700" : "hover:bg-slate-100"
              }`}
        ...
        <div className="grid grid-cols-3 gap-2">
          {Array.from({ length: 12 }, (_, month) => month).map((month) => {
            ...
            const startTime = new Date(Date.UTC(startYear, startMonth, 1)).getTime();
            const endTime = new Date(Date.UTC(endYear, endMonth + 1, 0)).getTime();
            const withinRange = candidate.getTime() >= startTime && candidate.getTime() <= endTime;
            return (
              <button
                key={month}
                type="button"
                className={`rounded-lg px-3 py-2 text-sm transition ${
                  month === activeMonth ? "bg-brand-100 text-brand-700" : "hover:bg-slate-100"
                } ${withinRange ? "" : "opacity-40 cursor-not-allowed"}`}
                disabled={!withinRange}
                onClick={() => withinRange && onSelect(activeYear, month)}
              >
                {UtcFormatter.formatShortMonth(new Date(Date.UTC(2000, month, 1)))}
              </button>
            );
          })}
        </div>
