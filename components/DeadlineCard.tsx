...
interface DeadlineCardProps {
  ...
  deadlineStatus?: 'expired_with_quorum' | 'expired_without_quorum' | 'active_with_quorum' | 'active_needs_quorum';
}

export default function DeadlineCard({ voteDeadline, phase, quorum, inCount, deadlineExpired }: DeadlineCardProps) {
  ...
  const deadline = new Date(voteDeadline);
  const expired = deadlineExpired !== undefined ? deadlineExpired : (isExpired || isAfter(new Date(), deadline));
      ...
      {expired && isVotePhase && needsMoreVotes && (
        <div className="mt-3 p-3 bg-red-100 rounded-lg">
          <p className="text-sm text-red-800 font-medium">
            ⚠️ Event failed: Deadline passed without reaching quorum ({inCount}/{quorum} votes)
          </p>
        </div>
      )}
      ...
      {!expired && isVotePhase && needsMoreVotes && (
        <div className="mt-3 p-3 bg-yellow-100 rounded-lg">
          <p className="text-sm text-yellow-800">
            ⏰ {quorum - inCount} more vote{quorum - inCount !== 1 ? 's' : ''} needed to proceed
          </p>
        </div>
      )}
