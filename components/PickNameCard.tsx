...
export default function PickNameCard({ token, attendeeNames, defaultTz, onJoined }: PickNameCardProps) {
  ...
  async function join() {
    ...
    try {
      ...
      console.log('Join successful, calling onJoined');
      console.log('🔍 PickNameCard: Join response status:', res.status);
      console.log('🔍 PickNameCard: Join response ok:', res.ok);
      
      // Add a small delay to ensure cache invalidation has time to complete
      await new Promise(resolve => setTimeout(resolve, 500));
      
      console.log('🔍 PickNameCard: About to call onJoined callback');
      onJoined();
      console.log('🔍 PickNameCard: onJoined callback called');
    } catch (err) {
    ...
    <div className="card grid gap-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-600">Choose your name and join the event</p>
        </div>
      </div>
      
      <div className="grid gap-4">
        <div className="grid gap-3">
          <label className="text-sm font-medium text-slate-700" htmlFor="nameOption">
            Pick your name
          </label>
          <select
            id="nameOption"
            ...
            value={slug}
            onChange={(event) => {
              const next = attendeeNames.find((name) => name.slug === event.target.value);
            ...
            {attendeeNames.map((name) => {
              ...
              return (
                <option key={name.id} value={name.slug} disabled={isTaken}>
                  {name.label}
                  {isClaimedByLoggedUser ? ' (claimed)' : isTaken ? ' (taken)' : ''}
                </option>
              );
            })}
          </select>
        ...
        <div className="grid gap-3">
          ...
          >
            ...
            {![
              ...
              'Europe/Berlin', 'Europe/Rome', 'Europe/Madrid', 'Asia/Tokyo', 'Asia/Shanghai', 
        ...
        {isClaimedByLoggedUser && (
          <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-yellow-700 font-medium">That name is claimed by a registered user. Only logged-in users can claim names.</span>
            </div>
          </div>
        )}
        ...
        {isTaken && !isClaimedByLoggedUser && (
          <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-red-700 font-medium">That name is currently taken. Please choose another name.</span>
            </div>
          </div>
        )}
