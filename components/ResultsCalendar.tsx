...
export default function ResultsCalendar({
  ...
  const canGoPrev = viewMonthStart.getUTCFullYear() > rangeStart.getUTCFullYear() || 
                   (viewMonthStart.getUTCFullYear() === rangeStart.getUTCFullYear() && 
                    viewMonthStart.getUTCMonth() > rangeStart.getUTCMonth());
  const canGoNext = viewMonthStart.getUTCFullYear() < rangeEnd.getUTCFullYear() || 
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600">
              {isInteractive ? "Click on any date to select it as your final date" : "View availability for all dates"}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-7 gap-1">
          {daysInGrid.map((day) => {
            ...
            if (!withinRange || muted) {
              ...
              cursorClass = isClickable ? "cursor-pointer hover:bg-green-200" : "";
            } else if (isEarliestMost) {
              bgColor = "bg-blue-100 border-blue-300";
              textColor = "text-blue-800";
              badgeText = "BEST";
              badgeColor = "bg-blue-600 text-white";
              cursorClass = isClickable ? "cursor-pointer hover:bg-blue-200" : "";
            } else if (withinRange && available > 0) {
              const ratio = available / totalAttendees;
              if (ratio >= 0.8) {
                bgColor = "bg-green-50 border-green-200";
                textColor = "text-green-700";
                cursorClass = isClickable ? "cursor-pointer hover:bg-green-100" : "";
              } else if (ratio >= 0.5) {
                bgColor = "bg-yellow-50 border-yellow-200";
                textColor = "text-yellow-700";
                cursorClass = isClickable ? "cursor-pointer hover:bg-yellow-100" : "";
              } else {
                bgColor = "bg-red-50 border-red-200";
                textColor = "text-red-700";
                cursorClass = isClickable ? "cursor-pointer hover:bg-red-100" : "";
              }
            } else if (withinRange) {
              bgColor = "bg-red-100 border-red-300";
              textColor = "text-red-800";
              cursorClass = isClickable ? "cursor-pointer hover:bg-red-200" : "";
            }
              ...
              >
                ...
                {withinRange && !muted && (
                  <div className="text-xs text-center">
                    <div className="font-medium">{available}/{totalAttendees}</div>
                    {badgeText && (
                      <div className={`inline-block px-1 py-0.5 rounded text-xs font-bold ${badgeColor} mt-1`}>
                        {badgeText}
                      </div>
                    )}
                  </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-700">Medium availability (50-79%)</span>
            </div>
            <div className="flex items-center gap-2">
        ...
        {isInteractive && (
          <div className="mt-3 pt-3 border-t border-slate-200">
            <p className="text-xs text-slate-600">
              💡 <strong>Tip:</strong> Click on any date within the event range to select it as your final date
            </p>
          </div>
        )}
      </div>
