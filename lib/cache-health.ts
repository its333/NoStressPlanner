...
export async function checkCacheHealth(): Promise<CacheHealthReport> {
  ...
  try {
    ...
    if (
      !eventGetResult ||
      JSON.stringify(eventGetResult) !== JSON.stringify(testValue)
    ) {
    ...
    if (
      !sessionGetResult ||
      JSON.stringify(sessionGetResult) !== JSON.stringify(testValue)
    ) {
      issues.push(
        'SessionCache GET operation failed or returned incorrect data'
      );
    }
    ...
    if (
      !availabilityGetResult ||
      JSON.stringify(availabilityGetResult) !== JSON.stringify(testValue)
    ) {
      issues.push(
        'AvailabilityCache GET operation failed or returned incorrect data'
      );
    }
    ...
    const totalHits =
      eventStats.hits + sessionStats.hits + availabilityStats.hits;
    const totalMisses =
      eventStats.misses + sessionStats.misses + availabilityStats.misses;
    const totalSets =
      eventStats.sets + sessionStats.sets + availabilityStats.sets;
    const totalDeletes =
      eventStats.deletes + sessionStats.deletes + availabilityStats.deletes;

    const totalOperations = totalHits + totalMisses + totalSets + totalDeletes;
    const hitRate =
      totalOperations > 0 ? (totalHits / totalOperations) * 100 : 0;
    const missRate =
      totalOperations > 0 ? (totalMisses / totalOperations) * 100 : 0;
 ...
 */
export async function validateEventCacheConsistency(
  token: string
): Promise<boolean> {
  try {
