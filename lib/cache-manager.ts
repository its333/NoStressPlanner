...
class CacheManager {
  ...
  private cleanupMemoryCache(): void {
    ...
    if (this.memoryCache.size > this.MEMORY_CACHE_MAX_SIZE) {
      ...
      const toRemove = entries.slice(
        0,
        entries.length - this.MEMORY_CACHE_MAX_SIZE
      );
  ...
  async get<T = any>(key: string, options?: CacheOptions): Promise<T | null> {
    ...
    try {
      ...
      if (redisManager.isHealthy()) {
        ...
        if (redisValue) {
          this.stats.hits++;
          debugLog('CacheManager: redis cache hit', {
      ...
      if (memoryData && !this.isExpired(memoryData.expires)) {
        this.stats.hits++;
        debugLog('CacheManager: memory cache hit', {
          cacheKey: `${cacheKey.substring(0, 50)}...`,
        });
      ...
      this.stats.misses++;
      debugLog('CacheManager: cache miss', {
        cacheKey: `${cacheKey.substring(0, 50)}...`,
      });
      return null;
    } catch (error) {
  ...
  async set<T = any>(
    ...
    try {
      ...
      // Set in Redis
      if (redisManager.isHealthy()) {
        const redisSuccess = await redisManager.set(
          cacheKey,
          serializedValue,
          ttl
        );
      ...
      this.stats.sets++;
      debugLog('CacheManager: cache set (memory)', {
        cacheKey: `${cacheKey.substring(0, 50)}...`,
        ttl,
      });
      return true;
  ...
  async delete(key: string, options?: CacheOptions): Promise<boolean> {
    ...
    try {
      ...
      this.stats.deletes++;
      debugLog('CacheManager: cache delete', {
        cacheKey: `${cacheKey.substring(0, 50)}...`,
      });
      return true;
    } catch (error) {
  ...
  async deletePattern(
    ...
    try {
      ...
      debugLog('CacheManager: cache delete pattern', {
        cachePattern: `${cachePattern.substring(0, 50)}...`,
        deletedCount,
      });
