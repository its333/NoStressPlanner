...
export async function getSessionKey(eventId?: string): Promise<string | undefined> {
  ...
  console.log('🔍 Cookie retrieval:', {
    hasNextAuthToken: !!sessionToken,
    hasAttendeeSession: !!attendeeSession,
    hasAnonymousSession: !!anonymousSession,
    nextAuthPreview: sessionToken ? `${sessionToken.substring(0, 20)}...` : null,
    attendeePreview: attendeeSession ? `${attendeeSession.substring(0, 20)}...` : null,
    anonymousPreview: anonymousSession ? `${anonymousSession.substring(0, 20)}...` : null,
  ...
  if (eventId && result) {
    ...
    if (result.includes(`_${eventPrefix}_`)) {
      console.log('🔍 Selected session key (event-specific):', `${result.substring(0, 20)}...`);
      return result;
    } else {
      console.log('🔍 Session key does not match event, ignoring:', `${result.substring(0, 20)}...`);
      return undefined;
    }
  }
  
  console.log('🔍 Selected session key (attendee only):', result ? `${result.substring(0, 20)}...` : 'none');
  return result;
}

export async function setSessionKey(sessionKey: string, sessionType: 'user' | 'anonymous' = 'anonymous'): Promise<void> {
  const cookieStore = await cookies();
  ...
  console.log('🔍 Set session key:', {
    ...
    cookieName: sessionType === 'user' ? 'attendee-session' : 'anonymous-session',
...
export async function clearAttendeeSessionCookies(): Promise<void> {
  ...
  console.log('🔍 AGGRESSIVELY cleared attendee session cookies');
}
